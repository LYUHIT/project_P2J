...
const appDB = {
    ...
    sp_CreateSchedule: async (circle_uid: number, user_uid: number, type: number, title: string, content: string, startTs: number, endTs: number) => {
        await db.runAsync("INSERT INTO tbl_Schedules (uuid, circle_uid, user_uid, type, title, content, start_time, end_time, updated_time, status) VALUES (?,?,?,?,?,?,?,?,?,?)", [
            String(Date.now()),
            circle_uid,
            user_uid,
            type,
    ...
    sp_UpdateSchedule: async (id: string, title: string, content: string, startTs: number, endTs: number) => {
        await db.runAsync("UPDATE tbl_Schedules SET title=?, content=?, start_time=?, end_time=?, updated_time=? WHERE uuid=?", [
            title,
            content,
            new Date(startTs).toISOString(),
            new Date(endTs).toISOString(),
    ...
    sp_DeleteSchedule: async (id: string) => {
        await db.runAsync("UPDATE tbl_Schedules SET status=9 WHERE uuid=?", [id]);
    },
